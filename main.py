...
from FLAlgorithms.servers.serverFedPA import FedPA
from FLAlgorithms.servers.serverFedHKD import FedHKD
...
def create_server_n_user(args, i):
    ...
    elif 'FedPA' in args.algorithm:
        server = FedPA(args, model, i)
    elif 'FedHKD' in args.algorithm:
        server = FedHKD(args, model, i)
...
def run_job(args, i):
    ...
    iid_phot = True
    if iid_phot and 'FedPA' in args.algorithm:
        server.visualize_iid_data()
    if args.train:
        server.train(args)
        server.test()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--algorithm", type=str, default="FedPA")
