...
class Generator(nn.Module):
    ...
    def init_loss_fn(self):
        ...
        self.L2_distanLoss = L2_distanLoss()
    ...
    def build_network(self):
        ...
        self.representation_layer = nn.Linear(self.fc_configs[-1], self.latent_dim) #linear 256-32
...
class DiversityLoss(nn.Module):
    ...
    def forward(self, noises, layer, y_input, isPA):
        """
        Forward propagation.
        """
        if len(layer.shape) > 2:
        ...
        if isPA:
            mask = torch.eq(y_input.unsqueeze(0), y_input.unsqueeze(1))
            layer_dist[mask] = 0
            noise_dist[mask] = 0
        return torch.exp(torch.mean(-noise_dist * layer_dist))

class L2_distanLoss(nn.Module):
    def __init__(self):
        """
        Class initializer.
        """
    ...
    def forward(self, tensor1, tensor2):
        l2_distances = torch.norm(tensor1 - tensor2,dim=1, p=2)
        return torch.mean(l2_distances)