...
import copy
import time
from visual_gene_data_TSNE import visual_gene_data_TSNE
...
class FedGen(Server):
    ...
    def train(self, args):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            if glob_iter == 0 or (glob_iter > 0 and glob_iter % 3 == 0):
                visual = visual_gene_data_TSNE(self.generative_model, glob_iter, self.algorithm, self.dataset, self.available_labels)
    ...
    def train_generator(self, batch_size, epoches=1, latent_layer_idx=-1, verbose=False):
        ...
        def update_generator_(n_iters, student_model, TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS):
            ...
            for i in range(n_iters):
                ...
                isPA = False
                diversity_loss=self.generative_model.diversity_loss(eps, gen_output, y_input, isPA)  # encourage different outputs
