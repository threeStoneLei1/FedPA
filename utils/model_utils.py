...
from FLAlgorithms.trainmodel.resnet18 import ResNet18
...
def get_data_dir(dataset):
    ...
    if 'EMnist' in dataset:
        ...
        dataset_=dataset.replace('alpha', '').replace('ratio', '').split('-')
        alpha, ratio=dataset_[1], dataset_[2]
        path_prefix=os.path.join('data', 'Mnist', 'u20c10-alpha{}-ratio{}'.format(alpha, ratio))  #run
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        ...
        path_prefix=os.path.join('data', 'CIFAR100', 'u20-c100-alpha{}-ratio{}'.format(alpha, ratio))
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/cifar100-n30/'

    elif 'CIFAR10' in dataset:
        dataset_=dataset.replace('alpha', '').replace('ratio', '').split('-')
        alpha, ratio=dataset_[1], dataset_[2]
        #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
        path_prefix=os.path.join('data', 'CIFAR10', 'u20-alpha{}-ratio{}'.format(alpha, ratio))
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
...
def create_generative_model(dataset, algorithm='', model='cnn', embedding=False):
    ...
    assert any([alg in algorithm for alg in ['FedPA', 'FedGen']])
    if 'FedGen' in algorithm or 'FedPA' in algorithm:
        ...
        if 'cnn' in algorithm:
            ...
            passed_dataset += '-cnn1'
    if dataset == 'twoVisualData':
        return Generator(passed_dataset, model=model, embedding=embedding, latent_layer_idx=0)
    else:
...
def create_model(model, dataset, algorithm):
    passed_dataset = get_dataset_name(dataset)
    if "cifar" in passed_dataset:
        model = ResNet18(passed_dataset),model
    else:
